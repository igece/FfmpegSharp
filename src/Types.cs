namespace FfmpegSharp
{
  ...
  public enum FileType
  {
    ...
    /// <summary>
    /// Audio Interchage File Format. Used on old Apple Macs, Apple IIc/IIgs and SGI. Ffmpeg’s AIFF support does not
    ...
    /// looking at its header to detect the true format. The sowt encoding is the only encoding that Ffmpeg can handle
    ...
    /// Ffmpeg can read native FLAC files (.flac) but not Ogg FLAC files (.ogg).
    /// Ffmpeg can write native FLAC files according to a given or default compression level
    ...
    /// encoded audio, Ffmpeg supports reading and writing only A-Law and μ-law.
    /// </summary>
    GSRT,
    ...
    /// for details. There is no associated file format, so Ffmpeg’s implementation is headerless.
    /// </summary>
    LPC,
    ...
    /// used with speech audio. Ffmpeg can read these files when they contain μ-law and PCM data. It will ignore any header
    ...
    /// not detected, nor are impossible sample rates. Ffmpeg supports reading (but not writing) VOC files with multiple
    ...
    /// Ffmpeg can read and write linear PCM, floating point, μ-law, A-law, MS ADPCM, and IMA (or DVI) ADPCM encoded
    /// samples. WAV files can also contain audio encoded in many other ways (not currently supported with Ffmpeg)
    /// e.g. MP3; in some cases such a file can still be read by Ffmpeg by overriding the file type. Big endian versions
    ...
    /// header, but in the mean time, this Ffmpeg format can be used to create files with the non-standard header that
    /// should work with these applications. Note that Ffmpeg will automatically detect and read WAV files with the
    /// non-standard header.
    /// </summary>
    WAVPCM,

    /// <summary>
    /// Psion 8-bit A-law. Used on Psion SIBO PDAs (Series 3 and similar). This format is deprecated in Ffmpeg, but will
  ...
  /// <summary>
  /// How Ffmpeg’s input combiner shall combine multiple input files.
  /// </summary>
  public enum CombinationType
  {
    ...
    /// stereo file. Un-merging is possible using multiple invocations of Ffmpeg with the remix effect.
    /// </summary>
    Merge,

    /// <summary>
    /// Mix multiple input files.
    /// The number of channels in each input file need not be the same, but Ffmpeg will issue a warning if they
